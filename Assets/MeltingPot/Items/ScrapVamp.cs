...
namespace MeltingPot.Items
{
    ...
    public class ScrapVamp : ItemBase<ScrapVamp> {
        ...
        public override string ItemName => "Scrap Vampire";
        public override string ItemLangTokenName => "SCRAPVAMP";
        public override string ItemPickupDesc => $"Defeating mechanical enemies rewards bonuses <style=cDeath>BUT all drones are hostile and spawn on sight</style>";
        public override string ItemFullDescription => $"Defeating types of mechanical enemies rewards permanent <style=cIsDamage>{damagePerStack}</style>/<style=cIsHealing>{healthPerStack}</style>/<style=cIsUtility>{armourPerStack}</style> <style=cStack>(per stack)</style>. All non-equipment drones spawn when within <style=cIsUtility>{baseRadius}m</style> <style=cStack>per stack</style>. All drones receive buffs at more than 1 stack.";
        public override string ItemLore => "[As you stare into it's eyes you hear a voice whisper]\n\n" +
        ...
        public static GameObject ItemModel => Assets.mainAssetBundle.LoadAsset<GameObject>("assets/meltingpot/mpassets/itemprefabs/scrap_vamp/ScrapVamp.prefab");
        public static GameObject ItemBodyModelPrefab;

        public static BuffDef VampArmour => ContentPackProvider.contentPack.buffDefs.Find("MeltingPot_VampArmour");
        public static BuffDef VampHealth => ContentPackProvider.contentPack.buffDefs.Find("MeltingPot_VampHealth");
        public static BuffDef VampDamage => ContentPackProvider.contentPack.buffDefs.Find("MeltingPot_VampDamage");
        public static ItemDef vampArmourTally => ContentPackProvider.contentPack.itemDefs.Find("ScrapVamp_ArmourTally");
        public static ItemDef vampHealthTally => ContentPackProvider.contentPack.itemDefs.Find("ScrapVamp_HealthTally");
        public static ItemDef vampDamageTally => ContentPackProvider.contentPack.itemDefs.Find("ScrapVamp_DamageTally");

        private EntityStates.SerializableEntityStateType droneStateOverride = new EntityStates.SerializableEntityStateType(typeof(NoRespawnDrone));
        ...
        public override ItemDisplayRuleDict CreateItemDisplayRules() {
            ItemBodyModelPrefab = Assets.mainAssetBundle.LoadAsset<GameObject>("assets/meltingpot/mpassets/itemprefabs/scrap_vamp/displayscrapvamp.prefab");
        ...
        private void CheckDeath(On.RoR2.GlobalEventManager.orig_OnCharacterDeath orig, RoR2.GlobalEventManager self, DamageReport rep) {
            if (rep.victimBody.baseNameToken.Contains("DRONE") && !rep.victimBody.baseNameToken.Contains("MEGADRONE")) {
            ...
            if ((((int)rep.victimBody.bodyFlags & 2) > 0) && GetCount(rep.attackerBody) > 0) {
                ...
                NetMessageExtensions.Send(new SyncVampStacks(rep.attackerBody.gameObject.GetComponent<NetworkIdentity>().netId, hStack, aStack, dStack), (NetworkDestination)(NetworkServer.active ? 1 : 2));
            }
            orig(self, rep);
        }
        private void AttachScrapVampCtrl(On.RoR2.CharacterBody.orig_OnInventoryChanged orig, global::RoR2.CharacterBody self) {

            self.AddItemBehavior<ScrapVampController>(base.GetCount(self));
            orig(self);
        }
        ...
        private class ScrapVampController : CharacterBody.ItemBehavior
        {
            ...
            private bool PurchaseInteractionIsMechanical(PurchaseInteraction target) {
                return target && target.available && (target.costType == CostTypeIndex.Money) && (target.displayNameToken.Contains("DRONE") || target.displayNameToken.Contains("TURRET") || target.displayNameToken.Contains("MEGA"));
            }
            ...
            public void FixedUpdate() {
                ...
                if (this.timerStopwatch <= 0f) {
                    ...
                    if (this.body.masterObject.GetComponent<CharacterMaster>()){
                        ...
                        CollectMinions(this.body.masterObject.GetComponent<CharacterMaster>(), CorruptAlly);
                    }
                }
