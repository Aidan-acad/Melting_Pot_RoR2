...
namespace MeltingPot.Items
{
    public class LeadFetters : ItemBase<LeadFetters>
    {
        ...
        public override string ItemPickupDesc => $"Increase <style=cIsUtility>Mass</style> by <style=cStack>0.1% (stacking hyperbolically)</style>. Increases <style=cIsUtility>Knockback Reduction</style> by <style=cStack>2.5% (stacking hyperbolically)</style>";

        public override string ItemFullDescription => $"Increase <style=cIsUtility>Fall Speed</style> by <style=cStack>0.1% (stacking hyperbolically)</style>. Increases <style=cIsUtility>Knockback Reduction</style> by <style=cStack>2.5% (stacking hyperbolically)</style>";
        ...
        {
            CreateItem("LeadFetters_ItemDef");
            CreateLang();
            Hooks();
        }
        ...
        private void ReduceKnockback(On.RoR2.HealthComponent.orig_TakeDamage orig, RoR2.HealthComponent self, RoR2.DamageInfo damageInfo)
        {
            ...
            if (InventoryCount > 0)
            {
                var percentReduction = Mathf.Clamp(1/(1+(InventoryCount * 0.025f)), 0, 1);
