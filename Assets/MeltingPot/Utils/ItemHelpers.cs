    ...
    public static class ItemHelpers
    {
        public static void DroneHunt(this CharacterMaster owner, Action<CharacterMaster> logic)
        {
            ...
            if (!thisMinionOwnership)
                return;
            MinionOwnership[] minionOwnerships =
                UnityEngine.Object.FindObjectsOfType<MinionOwnership>();
            foreach (MinionOwnership minionOwnership in minionOwnerships)
            {
                if (minionOwnership && minionOwnership.ownerMaster)
                {
                    bool ownerCondition =
                        !thisMinionOwnership.ownerMaster && minionOwnership.ownerMaster == owner;
                    bool minionCondition =
                        thisMinionOwnership.ownerMaster
                        && minionOwnership.ownerMaster == thisMinionOwnership.ownerMaster;
        ...
        )
        {
            ...
            CharacterModel.RendererInfo[] renderInfos = new CharacterModel.RendererInfo[
                AllRenderers.Count
            ];
            ...
            for (int i = 0; i < AllRenderers.Count; i++)
            {
                ...
                {
                    defaultMaterial =
                        AllRenderers[i] is SkinnedMeshRenderer
                            ? AllRenderers[i].sharedMaterial
