...
namespace MeltingPot.Items
{
    public class ThornShield : ItemBase<ThornShield>
    {
        ...
        public override string ItemFullDescription => $"Grants {flatArmour} armour per stack. When <style=cIsDamage>hit</style> in melee range, return <style=cStack>{flatDamage} + ({flatStack} per stack)</style> plus <style=cStack>{armourGrowth*100}% + ({armourGrowth*100}% per stack)</style> of your <style=cIsHealing>armour</style> value as damage";
                ...
                if (InventoryCount > 0) {
                    ...
                    if (distance < 5.0f) {
                        ...
                        DamageInfo reflect = new DamageInfo {
                            damage = flatDamage + flatStack*(InventoryCount-1) + victimBody.armor * (armourGrowth * InventoryCount),
                            crit = false,
