...
namespace MeltingPot.Items
{
    ...
    {
        public static BuffDef soulBurnBuff =>
            ContentPackProvider.contentPack.buffDefs.Find("MeltingPot_SoulBurn");
        public static BuffDef soulBurnSelfBuff =>
            ContentPackProvider.contentPack.buffDefs.Find("MeltingPot_SelfSoulBurn");
        public static BuffDef BurningSoulActiveBuff =>
            ContentPackProvider.contentPack.buffDefs.Find("MeltingPot_SoulBurnActive");
        ...
        public static int health_threshold = 1000;
        public static float enemy_burn_percent = 0.04f;
        public static float enemy_burn_percent_growth = 0.015f;
        public static float self_burn_percent = 0.01f;
        public static float self_burn_percent_growth = 0.005f;
        ...
        public class BurningSoulController : CharacterBody.ItemBehavior
        {
            ...
            public void FixedUpdate()
            {
                ...
                if (this.body.healthComponent.fullCombinedHealth > health_threshold)
                {
                    ...
                    if (getMaxHealth() != body.healthComponent.fullCombinedHealth)
                    {
                        ...
                        setInflictedDmg(
                            (float)Math.Ceiling(
                                (enemy_burn_percent + (stack - 1) * enemy_burn_percent_growth)
                                    * getMaxHealth()
                                    / 4
                            )
                        );
                    ...
                    setSelfDmg(
                        (float)Math.Floor(
                            body.healthComponent.combinedHealth
                                * (self_burn_percent + self_burn_percent_growth * (stack - 1))
                        )
                    );
            ...
            public void setInflictedDmg(float input)
            {
                healthdamage = input;
            }

            public void setSelfDmg(float input)
            {
                selfDamage = input;
            }
            ...
            public float getSelfBurn()
            {
                return timeSinceBurnt;
            }

            public void setSelfBurn(float input)
            {
                timeSinceBurnt = input;
            }
            ...
            public void setBurnt(bool input)
            {
                burnt = input;
            }
        }

        private void HandleBurningCtrl(
        ...
        {
            CreateItem("BurningSoul_ItemDef", enabled);
            if (enabled)
            {
                ItemModel = Assets.mainAssetBundle.LoadAsset<GameObject>(
                    $"{ModelPath}/burning_soul/burningsoul.prefab"
                );
                CreateLang();
                CreateEffect();
                CreateDoTs();
        ...
        public override ItemDisplayRuleDict CreateItemDisplayRules()
        {
            ItemBodyModelPrefab = Assets.mainAssetBundle.LoadAsset<GameObject>(
                $"{ModelPath}/burning_soul/displayburningsoul.prefab"
            );
            ...
            rules.Add(
                ...
                new RoR2.ItemDisplayRule[]
                {
                    new RoR2.ItemDisplayRule
                    {
                        ...
                        followerPrefab = ItemBodyModelPrefab,
                        childName = "Backpack",
                        localPos = new Vector3(-0.00751F, 0.40964F, 0.01287F),
                        localAngles = new Vector3(0F, 0F, 0F),
                        localScale = new Vector3(0.05097F, 0.05097F, 0.05097F)
                    }
                }
            );
            rules.Add(
                ...
                new RoR2.ItemDisplayRule[]
                {
                    new RoR2.ItemDisplayRule
                    {
                        ...
                        localPos = new Vector3(-0.08638F, -0.01881F, 0.14095F),
                        localAngles = new Vector3(352.3979F, 252.8352F, 265.8076F),
        ...
        public override void Hooks()
        {
            ...
            On.RoR2.Projectile.ProjectileManager.FireProjectile_FireProjectileInfo +=
                ProjectileDrain;
        }

        public void CreateEffect()
        {
            FireEffect = R2API.PrefabAPI.InstantiateClone(
            ...
            if (FireEffect)
            {
                R2API.PrefabAPI.RegisterNetworkPrefab(FireEffect);
            }
        ...
        private void CreateDoTs()
        {
            ...
            soulBurnDot = DotAPI.RegisterDotDef(
                sbDotDef,
                (dotController, dotStack) =>
                {
                    CharacterBody attackerBody =
                        dotStack.attackerObject.GetComponent<CharacterBody>();
                    if (
                        attackerBody
                        && attackerBody.gameObject.GetComponent<BurningSoulController>()
                    )
                    {
                        //float damageMultiplier = dmgCoefficient + dmgStack * (GetCount(attackerBody) - 1);
                        float burnDamage = attackerBody.gameObject
                            .GetComponent<BurningSoulController>()
                            .getInflictedDmg();
            ...
            soulBurnSelfDot = DotAPI.RegisterDotDef(
                sbSelfDotDef,
                (dotController, dotStack) =>
                {
                    CharacterBody attackerBody =
                    ...
                    {
                        float burnDamage = (float)Math.Ceiling(
        ...
        private bool MeleeDrain(
            On.RoR2.OverlapAttack.orig_Fire orig,
            RoR2.OverlapAttack self,
            List<RoR2.HurtBox> hitResults
        )
        {
                ...
                if (body)
                {
                    ...
                    if (InventoryCount > 0)
                    {
                        if (body.HasBuff(BurningSoulActiveBuff))
                        {
                            ...
                            if (!cooldownHandler)
                            {
                                cooldownHandler = owner.AddComponent<MeleeCooldownHandler>();
                            }
                            ...
                            if (!cooldownHandler.MeleeTracker.ContainsKey(self))
                            {
                                ...
                                if (!soulCtrl)
                                {
                                    soulCtrl = body.AddItemBehavior<BurningSoulController>(
                                        InventoryCount
                                    );
                                }
        ...
        private void ProjectileDrain(
            On.RoR2.Projectile.ProjectileManager.orig_FireProjectile_FireProjectileInfo orig,
            RoR2.Projectile.ProjectileManager self,
            RoR2.Projectile.FireProjectileInfo fireProjectileInfo
        )
        {
        ...
        {
            public Dictionary<RoR2.OverlapAttack, float> MeleeTracker =
                new Dictionary<RoR2.OverlapAttack, float>();
            ...
            if (attacker)
            {
                ...
                if (body && victimHealth)
                {
                    ...
                    if (InventoryCount > 0 && body.HasBuff(BurningSoulActiveBuff))
                    {
                        ...
                        effectData.SetHurtBoxReference(
                            victim.GetComponent<CharacterBody>().mainHurtBox
                        );
