...
namespace MeltingPot.Items
    ...
    public class ReactiveArmour : ItemBase<ReactiveArmour>
    {
        ...
        public override string ItemPickupDesc => $"After standing still for {timeout} second absorb <style=cStack>{2*absorbGrowth}% (+{absorbGrowth}% per stack)</style> of incoming damage. Using a skill whilst moving releases a nova.";

        public override string ItemFullDescription => $"After standing still for {timeout} second absorb <style=cStack>{2 * absorbGrowth}% (+{absorbGrowth}% per stack)</style> of incoming damage. Using a skill whilst moving releases a nova that deals <style=cIsDamage>{damageMult}x</style> the damage absorbed, in a {rangeMult}xdamage radius.";
        ...
        private class ArmourController : CharacterBody.ItemBehavior {
            ...
            public void addChargeStacks(Vector3 position) {
                ...
                body.SetBuffCount(NovaChargeBuff.buffIndex, body.GetBuffCount(NovaChargeBuff) + this.storedDamage);
            ...
            public void FixedUpdate() {
                ...
                bool flag2 = this.activeStopwatch <= 0f && !body.HasBuff(ReactiveArmour.NovaPreppedBuff) && !body.HasBuff(ReactiveArmour.NovaOffBuff);
                if (flag2)
                {
            ...
            if (owner)
            {
                ...
                if (InventoryCount > 0)
                {
                    ...
                    if (body.HasBuff(NovaChargeBuff) && body.HasBuff(NovaPreppedBuff))
                    {
                        ...
                        NetMessageExtensions.Send(new SyncRAState(body.gameObject.GetComponent<NetworkIdentity>().netId, true, true), (NetworkDestination)(NetworkServer.active ? 1 : 2));
                    }
                }
            }
            orig(self);
        ...
        private void TakeDamage(On.RoR2.HealthComponent.orig_TakeDamage orig, RoR2.HealthComponent self, RoR2.DamageInfo damageInfo)
        {
            ...
            if (!damageInfo.rejected || damageInfo == null)
            {
                ...
                if (self.body.HasBuff(NovaStationaryBuff) && inventoryCount > 0)
                {
                    ...
                    if (!armourCtrl) { orig(self, damageInfo); return; }
