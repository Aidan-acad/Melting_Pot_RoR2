...
namespace MeltingPot.Items
{
    public class LeadFetters : ItemBase<LeadFetters>
    {
        ...
        public override string ItemPickupDesc => $"Increase <style=cIsUtility>Armour</style> by <style=cIsUtility>{armourGrowth}</style> <style=cStack>(+{armourGrowth} per stack)</style>. Increases <style=cIsUtility>Knockback Reduction</style> by <style=cIsUtility>{knockbackGrowth*100}%</style> <style=cStack>(stacking hyperbolically)</style>";

        public override string ItemFullDescription => $"Increase <style=cIsUtility>Armour</style> by <style=cIsUtility>{armourGrowth}</style> <style=cStack>(+{armourGrowth} per stack)</style>. Increases <style=cIsUtility>Knockback Reduction</style> by <style=cIsUtility>{knockbackGrowth*100}%</style> <style=cStack>(stacking hyperbolically)</style>";

        public override string ItemLore => "[Etched onto the left cuff]\n\n" +
        ...
        public GameObject ItemModel => Assets.mainAssetBundle.LoadAsset<GameObject>("assets/meltingpot/mpassets/itemprefabs/lead_fetters/leadfetters.prefab");
        ...
        {
            ItemBodyModelPrefab = Assets.mainAssetBundle.LoadAsset<GameObject>("assets/meltingpot/mpassets/itemprefabs/lead_fetters/displayleadfetters.prefab");
        ...
        private void ReduceKnockback(On.RoR2.HealthComponent.orig_TakeDamage orig, RoR2.HealthComponent self, RoR2.DamageInfo damageInfo)
        {
            ...
            if (InventoryCount > 0)
            {
                var percentReduction = Mathf.Clamp(1/(1+(InventoryCount * knockbackGrowth)), 0, 1);
