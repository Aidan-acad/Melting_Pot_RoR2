...
namespace MeltingPot
{
    ...
    public class MeltingPotPlugin : BaseUnityPlugin
    {
        ...
        public static Dictionary<ItemBase, bool> ItemStatusDictionary =
            new Dictionary<ItemBase, bool>();
        ...
        private void Awake()
        {
            ...
            sotvRef = ExpansionCatalog.expansionDefs.FirstOrDefault(
                x => x.nameToken == "DLC1_NAME"
            );
        ...
        public static void ApplyShaders()
        {
            ...
            foreach (Material material in materials)
                if (material.shader.name.StartsWith("StubbedShader"))
                    material.shader = LegacyResourcesAPI.Load<Shader>(
                        "shaders" + material.shader.name.Substring(13)
                    );
        }
        ...
        public static void AttachControllerFinderToObjects(AssetBundle assetbundle)
        {
            ...
            {
                var foundRenderers = gameObject
                    .GetComponentsInChildren<Renderer>()
                    .Where(
                        x =>
                            x.sharedMaterial
                            && x.sharedMaterial.shader.name.StartsWith("Hopoo Games")
                    );
        ...
        private static void ApplyAllItemRules(On.RoR2.ItemDisplayRuleSet.orig_Init orig)
        {
            ...
            foreach (var bodyPrefab in BodyCatalog.allBodyPrefabs)
            {
                ...
                if (characterModel)
                {
                    ...
                    {
                        characterModel.itemDisplayRuleSet =
                            ScriptableObject.CreateInstance<ItemDisplayRuleSet>();
                    }
                    ...
                    foreach (ItemBase customItem in ItemStatusDictionary.Keys)
                    {
                        ...
                        if (customRules != null && customItem.Enabled)
                        {
                            ...
                            if (
                                customRules.TryGetRules(modelName, out rules)
                                || customRules.TryGetRules(bodyName, out rules)
                                || (
                                    allowDefault
                                    && characterModel.itemDisplayRuleSet
                                        .GetItemDisplayRuleGroup(customItem.ItemDef.itemIndex)
                                        .rules == null
                                )
                            )
                            {
                                characterModel.itemDisplayRuleSet.SetDisplayRuleGroup(
                                    customItem.ItemDef,
                                    new DisplayRuleGroup { rules = rules }
                                );
                            }
                        }
                    }
