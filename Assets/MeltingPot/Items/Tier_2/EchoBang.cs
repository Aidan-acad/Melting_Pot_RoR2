...
namespace MeltingPot.Items
{
    class EchoBang : ItemBase<EchoBang>
    {
        ...
        private class EchoController : CharacterBody.ItemBehavior
        {
            ...
            {
                BSModLogger = ModLogger;
                StoredEchos = StoredEchos
                    .Select(
                        c =>
                        {
                            c.Item2 -= Time.fixedDeltaTime;
                            return c;
                        }
                    )
                    .ToList();
        ...
        public override void Init(ConfigFile config, bool enabled)
        {
            ...
            if (enabled)
            {
                ItemModel = Assets.mainAssetBundle.LoadAsset<GameObject>(
                    $"{ModelPath}/reverb_ring/reverbring.prefab"
                );
                CreateLang();
                CreateEffect();
                Hooks();
        ...
        public override ItemDisplayRuleDict CreateItemDisplayRules()
        {
            ItemBodyModelPrefab = Assets.mainAssetBundle.LoadAsset<GameObject>(
                $"{ModelPath}/reverb_ring/displayreverbring.prefab"
            );
        ...
        private static GameObject LoadEffect(string soundName, bool parentToTransform)
        {
            GameObject newEffect = Assets.mainAssetBundle.LoadAsset<GameObject>(
                "assets/meltingpot/mpassets/effects/EchoRing.prefab"
            );
        ...
        public void CreateEffect()
        {
            ...
            if (EchoEffect)
            {
                PrefabAPI.RegisterNetworkPrefab(EchoEffect);
            }
